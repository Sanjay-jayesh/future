            ...
            >
                ...
                <div className="p-5 flex flex-col flex-1">
                    <div className="flex items-start justify-between gap-2 mb-2">
                        <h3 className="font-bold text-white group-hover:text-primary-400 transition-colors leading-snug">
                            {university.name}
                        </h3>
                        <StarRating rating={Math.round(university.rating)} size={14} />
                    </div>
                    <p className="text-sm text-gray-400 line-clamp-2 mb-3">{university.description}</p>
                    <div className="flex flex-wrap gap-1.5 mb-4">
                        {university.programs.slice(0, 3).map((program) => (
                            <span key={program} className="px-2 py-1 rounded-md bg-primary-600/15 text-primary-400 text-xs font-medium">
                                {program}
                            </span>
                        ))}
                        {university.programs.length > 3 && (
                            <span className="px-2 py-1 rounded-md bg-gray-800 text-gray-400 text-xs font-medium">
                                +{university.programs.length - 3} more
                            </span>
                        )}
                    </div>
                    <div className="mt-auto flex items-center justify-between pt-3 border-t border-gray-800">
                        <span className="text-sm font-semibold text-gray-200">{university.tuition}</span>
